/**

Caso de uso para la obtención de un cliente específico por su identificador.

Esta clase implementa el patrón de caso de uso, encapsulando la lógica de negocio

específica para la recuperación de un cliente individual. Coordina la operación

de consulta con el repositorio de persistencia, manteniendo una separación clara

de responsabilidades.

Responsabilidades:

Recibir el identificador único del cliente a consultar

Delegar la operación de búsqueda al repositorio

Retornar el cliente encontrado (o null si no existe) al controlador

Sigue el principio de inversión de dependencias (DIP) al recibir el repositorio

como dependencia inyectada, facilitando el testing unitario y posibles cambios

en el mecanismo de persistencia sin afectar la lógica de negocio.

@class GetClientById

@param {Object} clientRepository - Repositorio inyectado para operaciones de persistencia de clientes

@example

// Uso del caso de uso:

const getClientById = new GetClientById(clientRepository);

const cliente = await getClientById.execute(clientId);
*/

export default class GetClientById {
  constructor(clientRepository) {
    this.clientRepository = clientRepository;
  }

  async execute(id) {
    return await this.clientRepository.findById(id);
  }
}
